...
The [FRDR-demo](https://demo.frdr-dfdr.ca/repo/handle/doi:10.80217/demo.278?mode=full) site appears to be down (as of 6pm MST), so the package won't be able to download anything until it's back up. 

Changes in today's commit: Function `listdata_bc` has been revamped; now with cleaner and more detailed information printout (by default) and boolean return values as an alternative (mostly internal) feature. `datadir_bc` arguments have been simplified; and the interactive path selection feature has been removed to avoid platform-dependent complications. 
