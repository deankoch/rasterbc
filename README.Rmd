...
I have tried to keep dependencies to a minimum. Currently, the package requires: `rappdirs` (for an automated storage directory selection); and `sf`, `raster` (for loading and merging geospatial data). If these are not already installed on your machine, the `install_github` line will ask to install them.
