...
`rasterbc` provides a set of helper functions for accessing a large collection of spatial ecological data on the province of BC during the period 2001-2018, in a common (gridded) raster format. The goal is to make this type of data more accessible and easier to import into R, for the benefit of modelers interested in the forest ecology of BC.
...
* [Canadian national fire database, 2001-2018](https://github.com/deankoch/rasterbc_src/blob/master/src_nfdb.knit.md) ('fire'), from [Natural Resources Canada](https://cwfis.cfs.nrcan.gc.ca/ha/nfdb)
...
All raster data files will be hosted as a data publication (with associated DOI) on [FRDR](https://www.frdr-dfdr.ca/repo/) for permanence and easy referencing. 
...
To display the elevation data for the entire district, we need to combine the three blocks downloaded earlier. This can be done using `opendata_bc`, which
loads all required blocks, merges them into a single (mosaic) layer, crops and masks as needed, and then loads into memory the returned `RasterLayer` object:
...
The `listdata_bc` function can also be used to print a complete list of all available layers (use argument `verbose=2`). *eg.* in the 'dem' collection we also have 'aspect' and 'slope':

```{r listdata_bc-second, R.options = list(width = 15)}
...
If you're finished with `rasterbc` and want to remove all of the stored data, or if you simply want to free up space, the entire data directory or any of its contents can be deleted using your file browser. Alternatively the following R commmand can be used: 

```{r delete-example, R.options = list(width = 15), eval=FALSE}
unlink(getOption('rasterbc.data.dir'), recursive=TRUE)
```

This is quite safe unless you've set the data directory to something silly, like 'C:/', and will not break the `rasterbc` installation. However all downloaded data will be erased and you will need to run `datadir_bc` again before using the other package functions.
